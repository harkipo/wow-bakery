...
class OrderView(APIView):
    ...
    def post(self, request, format=None):
        try:
            ...
            total_value = 0
            final_items = []
            order = Order.objects.create(user=user,address=address,state=state,city=city,offer_id=offer_id)
            ...
            for item in items:
                ...
                order_item = OrderItems.objects.create(order=order,item_id=item_id,quantity=quantity)
                bakery_item = order_item.item
                bakery_item.quantity_sold = bakery_item.quantity_sold + quantity
                bakery_item.save()
                total_value = total_value + (int(order_item.item.sell_price) * int(quantity))
                order_item_serializer = OrderItemsSerializer(order_item,context={'request': request})
